    ...
    void shouldProcessPaymentCompletedEventAndUpdateRide() throws Exception {
        ...
        await().pollInterval(Duration.ofSeconds(1))
                ...
                .untilAsserted(() -> {
                    ...
                    assertEquals(PaymentStatus.PAID, updatedRide.getPaymentStatus());
                });
    }

    @Test
    @DisplayName("Должен принять событие PAYMENT_FAILED, пометить, что оплата поездки прошла неуспешно и сохранить +доп инфо")
    void shouldProcessPaymentFailedEventAndUpdateRide() throws Exception {
        // arrange
        RideEntity ride = RideEntity.builder()
                .passengerId(UUID.randomUUID())
                .status(RideStatus.COMPLETED)
        ...
        await().pollInterval(Duration.ofSeconds(1))
                ...
                .untilAsserted(() -> {
                    ...
                    assertEquals(PaymentStatus.FAILED, updatedRide.getPaymentStatus());
                });
    }
}